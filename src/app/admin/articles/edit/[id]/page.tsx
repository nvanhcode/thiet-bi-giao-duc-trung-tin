...
export default function EditArticlePage() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (resArticles.ok && resCategories.ok) {
        const dataArticles: Article[] = await resArticles.json();
        const dataCategories: ArticleCategory[] = await resCategories.json();

        setCategories(dataCategories);

        const target = dataArticles.find((a) => a.id === articleId);
        if (target) {
          setFormData(target);
        } else {
      ...
      showToast("Lỗi tải dữ liệu bài viết!", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedList = existingArticles.map((a) =>
  ...
  if (!formData) {
    return (
      <div className="max-w-xl mx-auto py-12 text-center space-y-4">
        <p className="text-base font-bold text-gray-800">Không tìm thấy thông tin bài viết này!</p>
        <Link
          href="/admin/articles"
          className="inline-flex items-center gap-2 px-4 py-2 bg-[#c8102e] text-white text-xs font-bold rounded-xl shadow"
        >
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-gray-900">Chỉnh Sửa Bài Viết</h2>
            <p className="text-xs text-gray-500">
              Cập nhật tiêu đề, danh mục, mô tả tóm tắt và nội dung WYSIWYG Rich Text Editor
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div className="md:col-span-2">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFormData((prev) => (prev ? { ...prev, title, slug: generateSlug(title) } : null));
                }}
            ...
            <div>
              ...
              <input
                ...
                value={formData.slug}
                onChange={(e) =>
                  setFormData((prev) => (prev ? { ...prev, slug: e.target.value } : null))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => (prev ? { ...prev, author: e.target.value } : null))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => (prev ? { ...prev, createdAt: e.target.value } : null))
                }
                className="w-full px-3.5 py-2 border rounded-xl text-xs outline-none focus:ring-2 focus:ring-[#c8102e]"
              />
            </div>
          ...
          <div className="pt-2 border-t border-gray-100">
            <label className="flex items-center gap-2 cursor-pointer font-bold text-gray-800 text-xs">
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => (prev ? { ...prev, featured: e.target.checked } : null))
                }
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-2">
          ...
          <textarea
            ...
            onChange={(e) =>
              setFormData((prev) => (prev ? { ...prev, excerpt: e.target.value } : null))
            }
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <RichTextEditor
            ...
            onChange={(html) =>
              setFormData((prev) => (prev ? { ...prev, content: html } : null))
            }
          />
        </div>
