...
export default function EditProductPage({ params }: PageProps) {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (prodRes.ok && catRes.ok && filterRes.ok) {
        const allProducts: Product[] = await prodRes.json();
        const foundProd = allProducts.find((p) => p.id === id);

        if (foundProd) {
          setFormData({
            ...foundProd,
            images: foundProd.images || [foundProd.thumbnail],
            specifications: foundProd.specifications || [],
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedProd = { ...formData, slug: currentSlug };

      const existsIndex = existingProducts.findIndex((p) => p.id === updatedProd.id);
      let updatedList = [...existingProducts];

      if (existsIndex >= 0) {
        updatedList[existsIndex] = updatedProd;
      } else {
        updatedList.push(updatedProd);
      }
  ...
  if (!formData) {
    return (
      <div className="p-8 text-center space-y-4">
        <p className="text-[#c8102e] font-bold">Không tìm thấy sản phẩm yêu cầu!</p>
        <Link href="/admin/products" className="text-xs text-blue-600 underline">
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-gray-900">Chỉnh Sửa Sản Phẩm: {formData.name}</h2>
            <p className="text-xs text-gray-500">
              Cập nhật thông tin chi tiết, mã sản phẩm, Rich Text Editor và thuộc tính bộ lọc
            </p>
          </div>
        </div>
